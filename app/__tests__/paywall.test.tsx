...
describe("Paywall", () => {
  ...
  // The hero leads with the one honest, checkable number this app can put on
  // a paywall — the deal archive depth — rather than a static claims list.
  it("leads with the deal-archive stat, not just a claims list", async () => {
    const { getByText } = await renderWithProviders(<Paywall />);
    expect(getByText("365")).toBeTruthy();
    expect(getByText(t("heroStatLine1"))).toBeTruthy();
    expect(getByText(t("heroStatLine2"))).toBeTruthy();
  });
