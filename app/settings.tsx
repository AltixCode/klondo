...
import { TABLE_THEME_NAMES, tableTheme } from "@/theme/table";
...
export default function Settings() {
  ...
  const tableName = useTableStore((s) => s.theme);
  const setTableTheme = useTableStore((s) => s.setTheme);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      <Screen scroll>
        ...
        <SectionLabel>{t("tableThemeTitle")}</SectionLabel>
        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.sm, marginTop: spacing.sm }}>
          {TABLE_THEME_NAMES.map((name) => {
            const selected = tableName === name;
            // Locked themes are shown, not hidden: nobody buys what they cannot see.
            const locked = !isPremium && name !== "default";
            const swatch = tableTheme(name);
            return (
              <Pressable
                key={name}
                accessibilityRole="radio"
                accessibilityLabel={
                  locked ? `${t(TABLE_LABEL[name]!)} — ${t("lockedTitle")}` : t(TABLE_LABEL[name]!)
                }
                accessibilityState={{ selected }}
                onPress={() => (locked ? router.push("/paywall") : setTableTheme(name, isPremium))}
                style={{
                  minHeight: 44,
                  flexDirection: "row",
                  alignItems: "center",
